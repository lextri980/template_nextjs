...
export default function ComponentName(props: TComponentNameProps) {
  ...
  // [Memo] Memoized values (if needed)
  const memoValue = useMemo(() => { ... }, [dependencies]);

  // [Effect] Side effects (if needed)
...
4. **[State]** - State declarations (useState)
5. **[Memo]** - Memoized values (useMemo, useCallback)
6. **[Effect]** - Side effects (useEffect)
7. **[Func]** - Helper functions and event handlers
8. **[JSX Section]** - Return statement (marked with `//!`)

### Comments

- Use structured comments with section markers:
  - `// [Props]` - Props destructuring section
  - `// [State]` - State declarations
  - `// [Memo]` - Memoized values
  - `// [Effect]` - Side effects
  - `// [Func]` - Functions section
  - `//! [JSX Section]` - JSX return section
...
2. Type imports (use `type` keyword)
3. Internal components (from `@/components`)
4. Internal utilities/hooks (from `@/hooks`, `@/utils`)
5. Local files (definition, styles)
